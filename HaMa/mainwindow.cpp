...
void MainWindow::computeSalaryStats()
{
    ...
    for (int row = 0; row < model->rowCount(); ++row) {
        QStandardItem* roleItem = model->item(row, 7); // Column 7 assumed for roles
    ...
    for (auto it = roleCounts.constBegin(); it != roleCounts.constEnd(); ++it) {
        ...
        QString label = QString("%1% (%2)").arg(QString::number(percentage, 'f', 1)).arg(it.key());
...
void MainWindow::computeSalaryState()
{
    ...
    // Iterate through the model to compute salary stats
    for (int row = 0; row < model->rowCount(); ++row) {
        QStandardItem* salaryItem = model->item(row, 7); // Column 7 assumed for salaries
        if (salaryItem) {
...
void MainWindow::sortTableBySalary()
{
    QStandardItemModel* model = dynamic_cast<QStandardItemModel*>(ui->xx1->model());
    if (model) {
        ui->xx1->setSortingEnabled(true); // Enable sorting for the view
        ui->xx1->sortByColumn(10, Qt::AscendingOrder); // Sort by Salary column (Column 10)
    } else {
...
void MainWindow::sortTableByDOB()
{
    ...
    if (model) {
        ui->xx1->setSortingEnabled(true); // Enable sorting for the view
        ui->xx1->sortByColumn(11, Qt::AscendingOrder); // Sort by DOB column (Column 11)
    } else {
        QMessageBox::warning(this, "Error", "Unable to sort. No valid model attached.");
    }
}
