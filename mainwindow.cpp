...

{
    ...
    // Connect the search button to the search function
        ...
        //--------------------------Gestion Stock------------------------------------------->
   /*
            connect(arduinoStock, &Arduino::dataReceived, this, &MainWindow::handleArduinoData);

        if (arduinoStock->connectArduino()) {
            connect(arduinoStock, SIGNAL(dataReceived(QString)), this, SLOT(updateDisplay(QString)));
        } else {
            QMessageBox::warning(this, "Error", "Could not connect to Arduino.");
        }*/

        // Additional setup for chartView and model if needed
        // setupChartView();
        //setupModel();

        connect(ui->Historique_Stock, &QPushButton::clicked, this, [this]() {
            ...
            ui->tableView_Histo->setModel(historyModel);
        });
        ...
        ui->code_uiStock->setValidator(numberValidator);
        ui->quantity_uiStock->setValidator(numberValidator);
        ...
        ui->name_uiStock->setValidator(letterValidator);
        ui->type_uiStock->setValidator(letterValidator);
...
void MainWindow::on_ajoute_Stock_clicked() {
    QString codeText = ui->code_uiStock->text();
    QString quantityText = ui->quantity_uiStock->text();
    QString nom_stock = ui->name_uiStock->text();
    QString type_stock = ui->type_uiStock->text();
    QDate date_stock = ui->date_uiStock->date();

    QString temperatureText = ui->tempeture_uiStock->text();
        // Assuming a QLineEdit for temperature

    // Ensure fields are filled
    if (codeText.isEmpty() || quantityText.isEmpty() || nom_stock.isEmpty() || type_stock.isEmpty() || temperatureText.isEmpty()) {
    ...
    double temperature = temperatureText.toDouble();

    // Create the stock object
    stock s(code_stock, quantity_stock, nom_stock, type_stock, date_stock, temperature);


    if (s.ajouter()) {
        QMessageBox::information(this, "Success", "Stock added successfully.");
        ui->tableViewStock->setModel(s.afficher());

        // Log history for the added stock
        QSqlQuery query;
        query.prepare("INSERT INTO STOCK_HISTORY (CODE_H, NOM_H, TYPE_H, QUANTITY_H, DATE_EXPIRATION_H, TEMPERATURE_H) "
                      "VALUES (:code, :nom, :type, :quantite, :date_expiration, :temperature)");
        ...
        //query.bindValue(":date_expiration", ui->date_expiration_ui->date());
    ...
    ui->code_uiStock->clear();
    ui->quantity_uiStock->clear();
    ui->name_uiStock->clear();
    ui->type_uiStock->clear();
    ui->tempeture_uiStock->clear();
    ui->date_uiStock->clear();
    //ui->date_expiration_ui->clear();
}




void MainWindow::on_delete_Stock_clicked()
{
    stock s;
    int code = ui->code_uiStock->text().toInt();
...
void MainWindow::on_show_Stock_clicked() {
    stock s;
    ui->tableViewStock->setModel(s.afficher());
}

void MainWindow::on_edit_Stock_clicked()
{
    ...
    QString nom_stock = ui->name_uiStock->text();
    QString type_stock = ui->type_uiStock->text();
    QDate date_stock = ui->date_uiStock->date();  // This will be used for DATE_EXPIRATION_H
    double temperature = ui->tempeture_uiStock->text().toDouble();  // Retrieve temperature from UI
    ...
    double oldTemperature = oldStock.getTemperature();  // Get the old temperature

    // Create a new stock object with the updated values
    stock s(code_stock, quantity_stock, nom_stock, type_stock, date_stock, temperature);  // Include temperature

    // Log the modification to stock_history table
    QSqlQuery queryHistory;
    queryHistory.prepare("INSERT INTO STOCK_HISTORY (CODE_H, NOM_H, TYPE_H, QUANTITY_H, DATE_EXPIRATION_H, TEMPERATURE_H) "
                         "VALUES (:CODE_H, :NOM_H, :TYPE_H, :QUANTITY_H, :DATE_EXPIRATION_H, :TEMPERATURE_H)");
    ...
    queryHistory.bindValue(":DATE_EXPIRATION_H", date_stock);  // Log the updated date_stock as expiration date
    queryHistory.bindValue(":TEMPERATURE_H", oldTemperature);  // Log the old temperature
    ...
    if (s.modifier()) {
        ...
        ui->tableViewStock->setModel(s.afficher());  // Update the stock table view
    } else {
    ...
    ui->name_uiStock->clear();
    ui->type_uiStock->clear();
    ui->tempeture_uiStock->clear();  // Clear temperature input field
    ui->date_uiStock->clear();
}

void MainWindow::on_Historique_Stock_clicked()
{
    // Create a query model for displaying the stock history
    QSqlQueryModel *historyModel = new QSqlQueryModel();

    // Prepare the SQL query to fetch all records including TEMPERATURE from STOCK_HISTORY
    historyModel->setQuery("SELECT CODE_H, NOM_H, TYPE_H, DATE_EXPIRATION, QUANTITE_H, TEMPERATURE_H FROM STOCK_HISTORY ORDER BY CODE_H DESC");
    ...
    historyModel->setHeaderData(5, Qt::Horizontal, "Temperature"); // New header for Temperature
...
{
    QString searchTerm = ui->searchInputStock->text();
    QString searchColumn;

    if (ui->radio_codeStock->isChecked()) {
        searchColumn = "CODE";
    } else if (ui->radio_nomStock->isChecked()) {
        searchColumn = "NOM";
    } else if (ui->radio_typeStock->isChecked()) {
        searchColumn = "TYPE";
    }
    ...
    if (query.exec()) {
        ...
        ui->tableView_TriStock->setModel(model);
    } else {
...
void MainWindow::on_PDF_Stock_clicked()
{
...
stock MainWindow::fetchStockByCode(int code_stock) {
    QSqlQuery query;
    query.prepare("SELECT NOM, QUANTITE, DATE_EXPIRATION, TEMPERATURE, TYPE FROM MEDICAMENTS WHERE CODE = :code");
    query.bindValue(":code", code_stock);

    if (query.exec() && query.next()) {
        QString nom = query.value("NOM").toString();
        int quantity = query.value("QUANTITE").toInt();
        QDate date_expiration = query.value("DATE_EXPIRATION").toDate();  // Fetch the expiration date
        double temperature = query.value("TEMPERATURE").toDouble();  // Fetch the temperature
        QString type = query.value("TYPE").toString();  // Fetch the type

        // Return the stock object with the correct parameters (date_expiration is omitted)
        return stock(code_stock, quantity, nom, type, date_expiration, temperature);
    }

    // Handle case where no data is found, possibly return an empty stock or handle error
    return stock();  // You might want to define how an empty stock object is returned
}
...
void MainWindow::on_notification_Stock_clicked()
{
    QVector<QPair<QString, int>> stockData = {
        {"Paracetamol", 0},
        {"Ibuprofen", 8},
...
void MainWindow::logStockHistory(int code, const QString &nom, const QString &type,
                                 int quantiteOld, int quantiteNew, const QDate &dateExpirationOld,
                                 const QDate &dateExpirationNew, double temperatureOld, double temperatureNew)
{
    // Prepare the SQL query to insert the stock change history into the database
    QSqlQuery query;
    query.prepare("INSERT INTO STOCK_HISTORY (CODE_H, NOM_H, TYPE_H, DATE_EXPIRATION_H, QUANTITY_H, TEMPERATURE_H) "
                  "VALUES (:CODE_H, :NOM_H, :TYPE_H, :DATE_EXPIRATION_H, :QUANTITY_H, :TEMPERATURE_H)");

    // Bind the values to the query
    query.bindValue(":CODE_H", code);  // Stock code
    query.bindValue(":NOM_H", nom);    // Stock name
    query.bindValue(":TYPE_H", type);  // Stock type
    query.bindValue(":DATE_EXPIRATION_H", dateExpirationNew);  // New expiration date
    query.bindValue(":QUANTITY_H", quantiteNew);  // New quantity
    query.bindValue(":TEMPERATURE_H", temperatureNew);  // New temperature

    // Execute the query
    if (!query.exec()) {
        qDebug() << "Error logging stock history: " << query.lastError().text();
    } else {
        qDebug() << "Stock history logged successfully.";
    }
}

void MainWindow::updateDisplay(float Humidity, float temperature)
{
    // Update the QLCDNumber widgets with the received data
    ui->lcdNumberStock->display(Humidity);      // Display humidity on lcdNumber_2
    ui->lcdNumberStock->display(temperature);     // Display temperature on lcdNumber

    // Display additional messages based on temperature
    if (temperature < 20.0)
    {
        ui->statusbar->showMessage("Medications are in a good environment :)");
    }
    else
    {
        ui->statusbar->showMessage("It's so hot! Be careful!");
    }

    qDebug() << "Display updated with Humidity:" << Humidity << "% and Temperature:" << temperature << "°C";
}
...
void MainWindow::handleArduinoData(QString data)
{
    ...
    // Replace tab characters with spaces to normalize the format
    data.replace("\t", " ");

    // Example data format: "Humidity: 51.10 % Temperature: 17.80 *C"
    QStringList dataList = data.split(' ');

    bool okHumidity = false, okTemperature = false;
    float humidity = 0.0, temperature = 0.0;

    // Parse humidity and temperature
    for (int i = 0; i < dataList.size(); ++i) {
        if (dataList[i] == "Humidity:") {
            humidity = dataList[i + 1].toFloat(&okHumidity);
        }
        if (dataList[i] == "Temperature:") {
            temperature = dataList[i + 1].toFloat(&okTemperature);
        }
    }

    // If parsing is successful, update the display
    if (okHumidity && okTemperature) {
        updateDisplay(humidity, temperature);
    } else {
        qDebug() << "Invalid data format received from Arduino:" << data;
    }
}
