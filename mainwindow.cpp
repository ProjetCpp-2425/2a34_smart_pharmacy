...
void MainWindow::afficherCommandesDansTable() {
    ...
    ui->tableWidget_5->setRowCount(model->rowCount());
    ui->tableWidget_5->setColumnCount(model->columnCount());

    // Set column headers
    ui->tableWidget_5->setHorizontalHeaderLabels({"ID Commande", "Nom Article", "Quantité", "Prix", "Date Livraison","Status"});

    for (int row = 0; row < model->rowCount(); ++row) {
        for (int col = 0; col < model->columnCount(); ++col) {
            QString data = model->index(row, col).data().toString();
            ui->tableWidget_5->setItem(row, col, new QTableWidgetItem(data));
        }
    }
}
...
void MainWindow::on_pushButton_3_clicked() {
    ...
    int quantite = ui->lineEdit_46->text().toInt();
    double prix = ui->lineEdit_48->text().toDouble();
    QDate dateLivraisonLivraison = ui->dateEdit->date();

    // Get the status from the combo box
    QString status = ui->lineEdit_47->text();
...
// recherche
void MainWindow::on_recherchComboBox_8_currentIndexChanged(int index) {
...
void MainWindow::on_pushButton_51_clicked() {
    QString searchCriteria = ui->comboBox_8->currentText();  // Get the selected search criteria
    QString inputValue = ui->edit_8->text();  // Get the user input from a line edit
    QSqlQuery query;
    ...
    if (query.exec()) {
        ...
        ui->tableWidget_5->setColumnCount(6);  // Adjust column count to 6 to accommodate STATUS
        ui->tableWidget_5->setHorizontalHeaderLabels({"ID Commande", "Nom Article", "Quantité", "Prix", "Date Livraison", "Status"});
...
void MainWindow::on_sortComboBox_4_currentIndexChanged(int index) {
    Commande Commande;
    QSqlQueryModel* model = nullptr;
    ...
    if (model) {
        ...
        for (int col = 0; col < model->columnCount(); ++col) {
            ui->tableWidget_5->setHorizontalHeaderItem(col, new QTableWidgetItem(model->headerData(col, Qt::Horizontal).toString()));
        }
        ...
        for (int row = 0; row < model->rowCount(); ++row) {
            ui->tableWidget_5->insertRow(row);
            for (int col = 0; col < model->columnCount(); ++col) {
                ui->tableWidget_5->setItem(row, col, new QTableWidgetItem(model->data(model->index(row, col)).toString()));
            }
        }
    } else {
...
void MainWindow::on_pushButton_52_clicked() {
    ...
    int index = ui->comboBox_4->currentIndex(); // Index-based approach
    switch (index) {
    case 0: // Sort by dateLivraison
    ...
    if (model != nullptr) {
        ...
        ui->tableWidget_5->setRowCount(0);  // Clear any existing rows
        ui->tableWidget_5->setColumnCount(model->columnCount());
...
void MainWindow::on_pushButton_39_clicked() {
    ...
    ui->stackedWidget->setCurrentIndex(6);  // Set to the main page (index 1)
...
void MainWindow::on_pushButton_26_clicked() {
    ...
    QString filePath = "C:/Users/Wissal/Downloads/2a34_smart_pharmacy-integrationfinal/2a34_smart_pharmacy-integrationfinal/list.pdf";
...
void MainWindow::on_pushButton_18_clicked() {
    ...
    if (userManager->login(username, password)) {
        ...
        ui->stackedWidget->setCurrentIndex(5); // Assuming index 2 is the main page
...
void MainWindow::on_pushButton_17_clicked() {
    ...
    if (userManager->signUp(username, email, password)) {
        ...
        ui->stackedWidget->setCurrentIndex(5); // Redirect to login page
    } else {
...
void MainWindow::showMainPage() {
    ...
    ui->stackedWidget->setCurrentIndex(5);  // Set to the main page (index 1)
}

void MainWindow::on_pushButton_19_clicked() {
    // Switch to login/sign-up page (index 0)
    ui->stackedWidget->setCurrentIndex(4);  // Go back to the login page

    // Optionally, you can clear any session data here if needed:
    ui->lineEdit_6->clear();  // Clear username field
    ui->lineEdit_7->clear();  // Clear password field
    ui->lineEdit_2->clear();  // Clear username field in sign-up
    ui->lineEdit_3->clear();  // Clear email field in sign-up
    ui->lineEdit_4->clear();  // Clear password field in sign-up
    ui->lineEdit_5->clear();  // Clear confirm password field in sign-up
}
...
{
    // Clear the layout before adding a new widget (chart)
    QLayout *layout = ui->dashboardWidget->layout();
    if (layout) {
        ...
        while ((item = layout->takeAt(0)) != nullptr) {
            ...
            delete item; // Clean up the layout item
        }
    }
    ...
    // Add the chart to the dashboard container (e.g., a QVBoxLayout in dashboardWidget)
    ui->dashboardWidget->layout()->addWidget(chartView);  // Add new chart widget
...
QSqlQueryModel* MainWindow::getCommandeMedicamentData() {
    QSqlQueryModel* model = new QSqlQueryModel();

    QSqlQuery query;
    query.prepare(
        "SELECT C.ID_COMMANDE, C.NOM_ARTICLE, C.QUANTITE, C.PRIX, C.DATE_LIVRAISON, C.STATUS, "
        "M.NOM AS MEDICAMENT_NOM, M.TEMPERATURE, "
        "CASE "
        "  WHEN TO_NUMBER(M.TEMPERATURE) > 15 THEN 'temperature non convenable' "
        "  ELSE 'temperature convenable' "
        "END AS TEMPERATURE_STATUS "
        "FROM COMMANDES C "
        "LEFT JOIN MEDICAMENTS M ON C.CODE_MEDICAMENT = M.CODE"
        );

    if (!query.exec()) {
        qDebug() << "Error retrieving commandes and medicaments:" << query.lastError().text();
        return nullptr;
    }
...
void MainWindow::loadCommandeData() {
    QSqlQueryModel* model = getCommandeMedicamentData();
    if (model) {
        ui->tableViewCommandes->setModel(model); // Set model for the table view
    }
}

void MainWindow::onReadyRead() {
    QByteArray data = serial.readAll();  // Read serial data
    QString message = QString::fromStdString(data.toStdString());
    qDebug() << "Received temperature message:" << message;

    // Extract temperature from the message (e.g., "TEMP:20")
    if (message.startsWith("TEMP:")) {
        float temperature = message.mid(5).toFloat(); // Extract number after "TEMP:"
        QString codeMedicament = "MED001"; // Example: use a specific code for testing

        // Update the database
        QSqlQuery query;
        query.prepare("UPDATE MEDICAMENTS SET TEMPERATURE = :temperature WHERE CODE = :code");
        query.bindValue(":temperature", QString::number(temperature));
        query.bindValue(":code", codeMedicament);

        if (query.exec()) {
            qDebug() << "Updated temperature in database successfully.";
            loadCommandeData(); // Refresh the view
        } else {
            qDebug() << "Failed to update temperature:" << query.lastError().text();
        }
    }
}
*/
